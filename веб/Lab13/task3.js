...
xhttp.open("GET","textx3.xml", true);
xhttp.send();
...
var mas = [];
    xhttp.onreadystatechange = function() {
            if (xhttp.status == 200 && xhttp.readyState == 4) {
                    ...
                    var book = textInf.getElementsByTagName("warehouse");

                    for(var i = 0; i < 5; i++){

                        mas.push(new Book(book[i].getAttribute("cod") ,book[i].getAttribute("name"), book[i].getAttribute("amount"),book[i].getAttribute("price")))
    ...
    xhttp.onreadystatechange = function() {
        if (xhttp.status == 200 && xhttp.readyState == 4) {
                ...
                for(var i = 0; i < 5; i++){
                    ...
                    mas.push(new Reader(reader[i].getAttribute("name") , reader[i].getAttribute("address"), reader[i].getAttribute("telephone")))
...
function createTableThird(){
    ...
    xhttp.onreadystatechange = function() {
        if (xhttp.status == 200 && xhttp.readyState == 4) {
                ...
                for(var i = 0; i < 5; i++){
                    mas.push(new R( new Book(book[i].getAttribute("author") ,book[i].getAttribute("title"), book[i].getAttribute("introduce"), book[i].getAttribute("price")), new Reader(reader[i].getAttribute("name") , reader[i].getAttribute("address"), reader[i].getAttribute("telephone")), r[i].getAttribute("firstdate"), r[i].getAttribute("lastdate"), r[i].getAttribute("date")));
                }
                var divEl = document.getElementById("divTask2");
                var table = document.createElement("table");
